...
TextSpan _highlightAll(
  ...
  final hl = base.copyWith(
    backgroundColor:
        active ? accent : DebugPalette.warning.withValues(alpha: 0.30),
...
class _HighlightedRawText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final line in lines) {
      final (span, hasActive, next) =
          _rawLineSpan(line, q, base, global, search.activeIndex, accent);
      global = next;
      final row = SelectableText.rich(span);
      rows.add(hasActive ? KeyedSubtree(key: search.activeKey, child: row) : row);
    }
    return Column(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        switch (_mode) {
          JsonViewMode.tree =>
            JsonObjectTree(widget.data, search: widget.search),
...
class JsonObjectTree extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final matches =
        search == null ? null : _collectTreeMatches(data, search!.query);
    return Container(
      width: double.infinity,
...
class _JsonNodeState extends State<_JsonNode> {
  ...
  int _matchIndex({required bool isKey}) {
    ...
    return matches.indexWhere(
      (m) => m.path == widget.path && m.isKey == isKey,
    );
  }
      ...
      child: Row(
        children: [
          ...
          if (widget.label.isNotEmpty)
            (keyIndex >= 0)
                ? Text.rich(_highlightAll('${widget.label}: ', search!.query,
                    labelStyle, active: keyActive, accent: accent))
  ...
  Widget _leaf(Object? v) {
    ...
    final row = Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: (valueIndex >= 0)
                ? SelectableText.rich(_highlightAll(
                    label, search!.query, valueStyle,
                    active: valueActive, accent: accent))
    ...
    return isActive
        ? KeyedSubtree(key: search!.activeKey, child: row)
        : row;
  }
